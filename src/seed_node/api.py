...
def get_utxos_address_view(address):
    utxos = read_db_json(get_cursor('utxos.db'), f'utxo_set WHERE pub_addr={address}', '*')
...
def validate_tx():
    ...
    if not verify_checksum(tx):
        raise ValueError('INVALID CHECKSUM: Checksum paramater does not match the checksum of the transaction')
        
    tx_signed_msg = tx_params[1]

    public_key = uncompress_public_key(tx_params[0])
    send_params = get_prefix_data(tx_params[1])

    tx_signed_msg = loads(exclude_keys(tx, {'signature'})).encode('utf-8')
    signature = b64decode(tx_signed_msg['signature'])

    verify_tx(tx_signed_msg, public_key, tx['version'], amount, tx['address'], inputs['inputs'], signature)

    for i in get_prefix_data(inputs):
        input_params = get_prefix_data(i)
        tx_id = input_params[0]
        output_index = input_params[1]

        del_db(utxos_cursor, 'utxo_set WHERE txid=? AND output_index=?', (tx_id, output_index))
